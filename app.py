...
from networksecurity.pipeline.trainint_pipeline import TrainingPipelien
...
async def train_route():
    try:
        train_pipeline = TrainingPipelien()
        train_pipeline.runPipeline()
        return Response("Traing is successfull")
...
async def predict_route(request: Request, file: UploadFile):
    try:
        ...
        preprocessing = load_object("final_model/preprocessor.pkl")
        final_model = load_object("final_model/model.pkl")
        network_model = NetworkModel(preprocessor=preprocessing, model=final_model)
